...
namespace Adaptive_Complex_detail {
  ...
  void alias_splitting_helper (std::list < std::list < GeoCell > > & answer, 
    ...
    unsigned int c = split_me . code & MASK; 
    ...
    while ( not work_stack . empty () ) {
      ...
      for ( unsigned int n = 0; n < Npow2; ++ n ) {
        ...
        for ( unsigned int i = 0; i < complex . dimension (); ++ complex . dimension () ) {
          if ( diff & 1 ) {
            /* b1_i != b2_i */
            a <<= 1;
            a |= ( choice & 1 );
  ...
  void alias_descent_helper (std::list < std::list < GeoCell > > & answer, 
                             ...
                             const Adaptive_Complex & complex ) {
    unsigned int MASK = 1 << complex . dimension () - 1;
    std::stack < std::pair <  Division_History::const_reverse_iterator, Cell > > work_stack;
    /* Initialize the stack with the first work item */ {
      GeoCell relative_state;
      relative_state . node = history . back () . node;
      relative_state . code = MASK << complex . dimension (); /* 111...1000...0 */
    ...
    while ( not work_stack . empty () ) {
      ...
      if ( second_iterator == history . rend () ) {
        ...
        translated . code =  first_iterator -> code ^ ( ( ~ relative_state . code >> complex . dimension () ) & MASK ); // side-by-side
        alias_splitting_helper ( answer, translated, complex . dimension () );
        continue;
      } /* if */
      ...
      if ( relative_state . node -> type == 1 ) { 
        ...
        if ( history . front () . code == first_iterator -> code ) {
          /* Perform Side-by-Side Translation */
          GeoCell translated = relative_state;
          translated . code =  first_iterator -> code ^ ( ( ~ relative_state . code >> complex . dimension () ) & MASK ); // side-by-side
          translated . code = complex . packing_code ( translated . code ); // Return back to packing code
          std::list < GeoCell > alias;
      ...
      unsigned int c1 = first_iterator -> code & MASK;
      
      unsigned int a2 = second_iterator -> node -> child_number;
      unsigned int b2 = second_iterator -> code >> complex . dimension ();
      unsigned int c2 = second_iterator -> code & MASK;
      
      unsigned int d = relative_state . node -> child_number;
      unsigned int s = relative_state . code >> complex . dimension ();
      //unsigned int t = relative_state . code & MASK;
  ...
  aliases (const GeoCell & initial_cell, const Adaptive_Complex & complex ) {
    ...
    while ( 1 ) {
      ...
      if ( dimension > old_dimension ) {
        ...
        alias_descent_helper ( return_value, history, complex . dimension () );
      } /* if */
  ...
  Node * operator ++ ( Node * & node ) {
    ...
    if ( node -> parent != NULL ) {
      ...
      while ( 1 ) {
        ...
        if ( which_child + 1 < reinterpret_cast < std::vector < Node * > * > ( node -> data ) -> size () )
          break;
      } /* while */
    } else {
...
Adaptive_const_iterator & Adaptive_const_iterator::operator ++ ( void ) {
  while ( 1 ) {
    ++ cell_ . code;
    if ( cell_ . code == container () . code_end ( dimension_ ) ) {
      ++ cell_ . node;
      ...
      cell_ . code = container () . code_begin ( dimension_ );
    } /* if */
    if ( cell_ . node -> data != NULL && reinterpret_cast < Cube_Cells * > ( cell_ . node -> data ) . read ( cell_ . code ) ) return *this;
  } /* while */
...
Adaptive_Complex::Chain Adaptive_Complex::boundary ( const Adaptive_const_iterator & input ) const {
  ...
  Adaptive_Complex::Chain output;
    ...
    if ( ( ( cell . code >> ( dimension_ + dimension_index ) ) & 1 ) == 1  ) { 
      ...
      /* Produce Left Cell */
      lower_cell . code = cell . code & ( not ( 1 << ( dimension_ + dimension_index ) ) ) & ( not ( 1 << dimension_index ) );
      /* Process Left Cell */ {
        std::list < std::list < GeoCell > > left_boundary_aliases = aliases ( lower_cell );
        /*Process the chain*/
        BOOST_FOREACH ( std::list < GeoCell > & alias, left_boundary_aliases ) {
          BOOST_FOREACH ( GeoCell & alias_cell, alias ) {
            if ( alias_cell . node -> data != NULL && reinterpret_cast < Cube_Cells * > ( alias_cell . node -> data ) 
                 -> read ( complex . packing_code ( alias_cell . code ) ) )
      ...
      /* Process Right Cell */ {
        std::list < std::list < GeoCell > > right_boundary_aliases = aliases ( lower_cell );
        /*Process the chain*/
        BOOST_FOREACH ( std::list < GeoCell > & alias, right_boundary_aliases ) {
          BOOST_FOREACH ( Cell & alias_cell, alias ) {
            if ( alias_cell . node -> data != NULL && reinterpret_cast < Cube_Cells * > ( alias_cell . node -> data ) 
                 -> read ( complex . packing_code ( alias_cell . code ) ) )
              output . insert ( std::pair < const_iterator, Ring > ( const_iterator ( *this, alias_cell ), coincidence_index ) );
          } /* BOOST_FOREACH */
...
Adaptive_Complex::Chain Adaptive_Complex::coboundary ( const Adaptive_const_iterator & input) const {
  ...
  Adaptive_Complex::Chain output;
	int coincidence_index = 1;
  GeoCell cell ( *input, *this );
  std::list < std::list < GeoCell > > cell_aliases = aliases ( cell );
	/*Go thru all cells which are one dimension lower than input*/
	for ( unsigned int dimension_index = 0; dimension_index < dimension_; ++ dimension_index ) {
      ...
      BOOST_FOREACH ( std::list < GeoCell > & alias, cell_aliases ) {
        BOOST_FOREACH ( GeoCell & alias_cell, alias ) {
          ...
          if ( coboundary_cell . node -> data != NULL && reinterpret_cast < Cube_Cells * > ( coboundary_cell . node -> data )
               -> read ( complex . packing_code ( coboundary_cell . code ) ) ) {
...
Adaptive_Complex::Adaptive_Complex ( unsigned int complex_dimension ) : dimension_(complex_dimension)  {
  /* Initialize Tree */
	root_ = new Node ( 0 /* Leaf */, dimension_ );
  ...
  BOOST_FOREACH ( unsigned int child_number, splitting ) {
    if ( node -> type == 0 ) {
      ...
      BOOST_FOREACH ( Node * & child, * reinterpret_cast < std::vector<Node *> * > ( node -> data ) ) {
        child = new Node ( 0, dimension_ );
      } /* BOOST_FOREACH */
    } /* if */
    node = reinterpret_cast < std::vector<Node *> * > ( node -> data ) -> operator [] ( child_number );
  } /* BOOST_FOREACH */
  reinterpret_cast < std::vector<Node *> * > ( node -> data ) -> operator [] ( child_number ) -> data =
    new Cube_Cells ( dimension_ );
  return true;
} /* Adaptive_Complex::Add_Full_Cube */

namespace Adaptive_Complex_detail {

  void finalize_helper ( const Cell /* geometric code */ & input, const unsigned int full_dimension ) {
    /* Add the cell unless there is a conflict */
    std::list < std::list < Cell > > cell_aliases = aliases ( input );
    /* Erase self-alias from answer */
    cell_aliases . erase ( cell_aliases . begin () );
    /* Search for a conflict */
    bool conflict = false;
    BOOST_FOREACH ( std::list < Cell > & alias, cell_aliases ) {
      /* Check if any of the Cell's in this alias are "on" */
      BOOST_FOREACH ( Cell & cell, alias ) {
        if ( cell . node -> data != NULL && reinterpret_cast < Cube_Cells * > ( cell . node -> data ) -> read ( cell . code ) ) {
          conflict = true;
          break;
        } /* if */
      } /* BOOST_FOREACH */
      if ( conflict == true ) {
        BOOST_FOREACH ( Cell & cell, alias ) {
          if ( cell . node -> data != NULL && reinterpret_cast < Cube_Cells * > ( cell . node -> data ) -> read ( cell . code ) ) continue;
          Cell geo_cell = cell;
          geo_cell . code = complex . geometric_code ( geo_cell . code );
          finalize_helper ( geo_cell, full_dimension );
        } /* BOOST_FOREACH */
        return;
      } /* if */
    } /* BOOST_FOREACH */
    ...
    /* Write the Cell */
    if ( input . node -> data == NULL ) input . node -> data = new Cube_Cells ( full_dimension );
    ...
    for ( unsigned int dimension_index = 0; dimension_index < dimension_; ++ dimension_index ) {
      ...
      if ( ( ( cell . code >> ( dimension_ + dimension_index ) ) & 1 ) == 1  ) { 
        ...
        /* Produce Left Cell */
        lower_cell . code = cell . code & ( not ( 1 << ( dimension_ + dimension_index ) ) ) & ( not ( 1 << dimension_index ) );
        finalize_helper ( lower_cell, full_dimension );
        /* Produce Right Cell */
        lower_cell . code = lower_cell . code | ( 1 << dimension_index );
        finalize_helper ( lower_cell, full_dimension );
      } /* if */
...
void Adaptive_Complex::Finalize ( void ) {
  ...
  /* Produce the ordered list of leaves */ {
    std::map < unsigned int, std::list < Node * >::iterator > position_map;
    Node * leaf = root_;
    while ( ++ leaf != root_ ) {
  ...
  BOOST_FOREACH ( Node * leaf, work_list ) {
    Cell top ( leaf, ( 1 << dimension_ - 1 ) << dimension_ );
    finalize_helper ( top );
  } /* BOOST_FOREACH */
  ...
  if ( begin_cell . node -> data == NULL || not reinterpret_cast < Cube_Cells * > ( begin_cell . node -> data ) -> read ( begin_cell . code ) ) {
    ++ begin_ [ 0 ];
  } /* if */
