...
void Dense_Matrix_Boundary_Map ( typename Dense < typename Cell_Complex::Ring >::Matrix & output_matrix,
    ...
    if ( dimension == 0 ) {
        output_matrix = typename Dense < typename Cell_Complex::Ring >::Matrix ( 0, complex . Chain_Groups [ dimension ] . size () );
        return;
    } else if ( dimension == complex . dimension + 1 ) {
        output_matrix = typename Dense < typename Cell_Complex::Ring >::Matrix ( complex . Chain_Groups [ dimension - 1 ] . size (), 0 );
        return;
    } else
        output_matrix = typename Dense < typename Cell_Complex::Ring >::Matrix ( complex . Chain_Groups [ dimension - 1 ] . size (),
                                                                        complex . Chain_Groups [ dimension ] . size () );
    
    
    /* First we make the translation table. */
    translation_table . resize ( complex . Chain_Groups [ dimension - 1 ] . size () + 1 );
    std::map < typename Cell_Complex::Elementary_Chain, unsigned long >  inverse_table;
    /* scope */ { 
    unsigned long index = 0;
    for ( typename Cell_Complex::const_iterator cell = complex . Chain_Groups [ dimension - 1 ] . begin (); 
    cell != complex . Chain_Groups [ dimension - 1 ] . end (); ++ cell ) {
        ++ index;
        translation_table [ index ] = cell -> first; 
        inverse_table [ cell -> first ] = index;
    } /* for */
    ...
    unsigned long index = 0;
    for ( typename Cell_Complex::const_iterator right_cell = complex . Chain_Groups [ dimension ] . begin (); 
    right_cell != complex . Chain_Groups [ dimension ] . end (); ++ right_cell ) {
        ++ index;
